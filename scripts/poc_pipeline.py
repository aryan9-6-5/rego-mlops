...
def calculate_confidence(raw_text: str, candidate_formula: str, z3_parse_result: bool) -> dict:
    """
    ...
    terms_captured = sum(1 for term in key_terms if term.lower() in candidate_formula.lower())
    ...
    z3_vars = re.findall(r"Real\('(\w+)'\)", candidate_formula) + re.findall(r"Bool\('(\w+)'\)", candidate_formula)
...
Regulation: {RAW_RBI_TEXT}"""
    ...
    if len(formula_lines) > 2:
        display_formula = formula_lines[-1] if "rule" in formula_lines[-1] else formula_lines[0] + "..."
    else:
    ...
    print(f"    completeness: {confidence['signals']['completeness']:.2f} | z3_valid: {confidence['signals']['z3_valid']:.2f} | specificity: {confidence['signals']['specificity']:.2f}\n")
    ...
    # Step 4: Proof
    print("Step 4: Proof")
    proof_str = candidate_formula + str(model_a) + str(model_b) + str(a_compliant) + str(b_compliant)
